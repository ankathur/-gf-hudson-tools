#!/bin/bash
DEFAULT_IMAGE_NAME=gf-jenkins/generic-salve-new:v0
...
if [ `uname | grep -i "sunos" | wc -l | awk '{print $1}'` -eq 1 ] ; then
 ...
 while : ; do
  ...
  docker run ${NAME_ARG} --privileged=true ${NAME_ARG} ${GLOBAL_ENVS} ${JENKINS_VARS} ${ENV_PARAMS} -e PATH="$PATH:${JAVA_HOME}\bin" -e MAVEN_OPTS="${MAVEN_OPTS}"  -e TEST_IDS="${TEST_IDS}" -e EXECUTE_SCRIPT="${EXECUTE_SCRIPT}" -v /scratch/BUILD_AREA:/scratch/BUILD_AREA -v /tmp/auto_${BUILD_NUMBER}.log:/var/log/automount.log -h ${HOST} "${dn_opt}" ${IMAGE_NAME}
  result=$?
